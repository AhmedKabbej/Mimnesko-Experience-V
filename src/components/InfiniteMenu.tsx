  ...
  subdivide(divisions = 1): this {
    ...
    for (let div = 0; div < divisions; ++div) {
      ...
      f.forEach((face) => {
        ...
        newFaces.push({ a: face.a, b: mAB, c: mCA });
        newFaces.push({ a: face.b, b: mBC, c: mAB });
        newFaces.push({ a: face.c, b: mCA, c: mBC });
        newFaces.push({ a: mAB, b: mBC, c: mCA });
      });
...
class IcosahedronGeometry extends GeometryClass {
  constructor() {
    super();
...
class DiscGeometry extends GeometryClass {
...
class InfiniteGridMenu {
  ...
  private init(onInit?: (menu: InfiniteGridMenu) => void): void {
  ...
  private initTexture(): void {
    ...
    canvasEl.width = this.atlasSize * cellSize;
    canvasEl.height = this.atlasSize * cellSize;
...
export default function InfiniteMenu({ items = [], scale = 1.0, onActionClick }: { items: MenuItem[]; scale?: number; onActionClick?: (item: MenuItem) => void }) {
